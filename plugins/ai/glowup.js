// plugins/ai/glowup.js — "glow up" foto (versi lebih putih/cerah), pakai endpoint
// GET /faa/toputih?url=... dari api-faa.my.id.
...
let handler = async (m, { sock, prefix, command }) => {
  ...
  try {
    ...
    await sock.sendMessage(m.chat, { text: ok("Selesai glow up!"), edit: statusMsg.key });
    await sock.sendMessage(m.chat, { image: imageBuffer }, { quoted: m });
  } catch (err) {
    console.error("[GLOWUP GAGAL]", err?.message || err);
    try { await sock.sendMessage(m.chat, { text: fail("Gagal glow up — API FAA lagi down/error. Coba lagi nanti."), edit: statusMsg.key }); } catch {}
  }
};

handler.command = ["glowup"];
handler.help = ["glowup (reply foto — bikin glow up/lebih cerah)"];
handler.tags = ["ai"];
