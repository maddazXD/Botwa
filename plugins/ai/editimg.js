// plugins/ai/editimg.js — edit foto pakai instruksi teks bebas, pakai endpoint
// GET /faa/editfoto?url=...&prompt=... dari api-faa.my.id.
...
let handler = async (m, { sock, text, prefix, command }) => {
  ...
  if (!source || source.mtype !== "imageMessage" || !prompt) {
    return m.reply(
      usage(`Reply FOTO, terus ketik ${prefix}${command} <instruksi edit>`, `${prefix}${command} ganti background jadi pantai`)
    );
  }

  const label = `Lagi ngedit foto: "${prompt}"...`;
  const statusMsg = await sock.sendMessage(m.chat, { text: processing(label) }, { quoted: m });

  try {
    const buffer = await source.download();
    let lastPercent = -1;
    const imageBuffer = await faaImageTransform(uploadImageBuffer, buffer, "/faa/editfoto", { prompt }, "url", {
    ...
    console.error("[EDITIMG GAGAL]", err?.message || err);
...
handler.command = ["editimg", "editfoto"];
handler.help = ["editimg <instruksi> (reply foto — edit foto pakai teks bebas)"];
handler.tags = ["ai"];
