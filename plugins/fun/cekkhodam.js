// plugins/fun/cekkhodam.js — Cek "khodam" random (lucu-lucuan)
let khodamCache = {};
...
let handler = async (m, { text }) => {
  ...
  const data = khodamCache[text] || pickRandom(khodamList);
  khodamCache[text] = data;

  const result = `👤 Nama: *${text}*\n🔮 Khodam kamu adalah: *${data.nama}*\n\n📖 Penjelasan:\n${data.deskripsi}`.trim();
  await m.reply(result);
};

handler.command = ["cekkhodam", "cekkodam", "kodam", "khodam"];
handler.tags = ["fun"];
